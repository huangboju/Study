...
protocol AppHelperDelegate: class {
    func AppHelperPrint(_ log: String)
}
...
extension AppHelper {
    static var resourcesBundle: Bundle? {
        return AppHelper.resourcesBundle(with: QMUIResourcesMainBundleName)
    }

    static func image(with name: String) -> UIImage? {
        let bundle = AppHelper.resourcesBundle
        return AppHelper.image(in: bundle, with: name)
    }
...
extension AppHelper {
    ...
    static func heightForDynamicTypeCell(_ heights: [CGFloat]) -> CGFloat {
        let index = AppHelper.preferredContentSizeLevel
...
extension AppHelper {
    ...
    func handleKeyboardWillShow(notification: Notification) {
        ...
        lastKeyboardHeight = AppHelper.keyboardHeight(with: notification)
    }
    ...
    public static func keyboardHeight(with notification: Notification) -> CGFloat {
        return AppHelper.keyboardHeight(with: notification, in: nil)
    }
    ...
    public static func keyboardAnimationOptions(with notification: Notification) -> UIViewAnimationOptions {
        let rawValue = UInt(AppHelper.keyboardAnimationCurve(with: notification).rawValue)
...
extension AppHelper {
    ...
    public static var isIPadPro: Bool {
        return AppHelper.isIPad && (DEVICE_WIDTH == 1024 && DEVICE_HEIGHT == 1366)
    }
...
extension AppHelper {
    ...
    public static var transformForCurrentInterfaceOrientation: CGAffineTransform {
        return AppHelper.transformWithInterface(orientation: UIApplication.shared.statusBarOrientation)
    }
    ...
    public static func transformWithInterface(orientation: UIInterfaceOrientation) -> CGAffineTransform {
        ...
        let angle = AppHelper.angleForTransformWithInterface(orientation: orientation)
...
class AppHelper: NSObject {
    ...
    weak var helperDelegate: AppHelperDelegate?
